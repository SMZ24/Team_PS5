...
void AVLTree::inorderTraversal(AVLNode* node) {
    if (node != nullptr) {
        ...
        std::cout << "ID: " << node->getStudentID() 
                  << " Name: " << node->getName() 
                  << " Course: " << node->getCourse() 
                  << " Grade: " << node->getGrade() 
                  << " GPA: " << node->getGPA() << std::endl;
        inorderTraversal(node->getRight());
    }
}

int AVLTree::compareKeys(int id1, std::string course1, int id2, std::string course2) {
    if (id1 < id2) return -1;
    if (id1 > id2) return 1;
...
AVLNode* AVLTree::insertHelper(AVLNode* node, int student_ID, std::string name, std::string course, float grade, float GPA) {
    if (node == nullptr) {
        return new AVLNode(student_ID, name, course, grade, GPA);
    }

    int cmp = compareKeys(student_ID, course, node->getStudentID(), node->getCourse());
    
    if (cmp < 0) {
        node->setLeft(insertHelper(node->getLeft(), student_ID, name, course, grade, GPA));
    } else if (cmp > 0) {
        node->setRight(insertHelper(node->getRight(), student_ID, name, course, grade, GPA));
    } else {
        return node;
    }
    ...
    if (balance > 1) {
        int cmpLeft = compareKeys(student_ID, course, node->getLeft()->getStudentID(), node->getLeft()->getCourse());
        if (cmpLeft < 0) {
    ...
    if (balance < -1) {
        int cmpRight = compareKeys(student_ID, course, node->getRight()->getStudentID(), node->getRight()->getCourse());
        if (cmpRight > 0) {
...
void AVLTree::collectGrades(AVLNode* node, int student_ID, float& total, int& count) {
    if (node == nullptr) {
        return;
    }
...
bool AVLTree::insert(int student_ID, std::string name, std::string course, float grade) {
...
AVLNode* AVLTree::searchHelper(AVLNode* node, int student_ID, std::string course) {
    if (node == nullptr) {
        return nullptr;
    }
...
AVLNode* AVLTree::deleteHelper(AVLNode* node, int student_ID, std::string course) {
    if (node == nullptr) {
        return node;
    }
    ...
    if (cmp < 0) {
        ...
        node->setRight(deleteHelper(node->getRight(), temp->getStudentID(), temp->getCourse()));
    }
    ...
    for (size_t i = 0; i < courses.size(); i++) {
        ...
        if (i == 0) {
            std::cout << std::left << std::setw(8) << node->getStudentID()
                      << std::setw(20) << node->getName()
                      << std::setw(15) << node->getCourse()
                      << std::setw(10) << std::fixed << std::setprecision(1) << node->getGrade()
                      << std::setw(8) << std::fixed << std::setprecision(2) << node->getGPA() << "\n";
        } else {
...
void AVLTree::saveToFileHelper(AVLNode* node, std::ofstream& file) {
    ...
    file << node->getStudentID() << "," << node->getName() << "," << node->getCourse() << "," << node->getGrade() << "," << node->getGPA() << std::endl;
