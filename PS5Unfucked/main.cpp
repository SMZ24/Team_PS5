...
float getFloatInput(const std::string& prompt, float min = 0.0f, float max = 100.0f) {
    float value;
    while (true) {
        std::cout << prompt;
        if (std::cin >> value) {
            if (value >= min && value <= max) {
                ...
                std::cout << "Error: Grade must be between " << min << " and " << max << ". Please try again.\n";
                std::cin.clear();
                std::cin.ignore(10000, '\n');
            }
        } else {
...
std::string getStringInput(const std::string& prompt, bool isName = false) {
    ...
    while (true) {
        ...
        if (isName && !isValidName(value)) {
            std::cout << "Error: Name can only contain letters, spaces, and hyphens. Please try again.\n";
            continue;
        }
        if (!isName && !isValidCourse(value)) {
            std::cout << "Error: Course name can only contain letters, numbers, spaces, and hyphens. Please try again.\n";
            continue;
        }
        return value;
    }
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            if (tree.insert(student_ID, name, course, grade)) {
                ...
                std::cout << "Error: Student ID already exists with a different name.\n";
            }
        }
        else if (choice == 2) {
            student_ID = getIntInput("Enter Student ID: ", 1);
